...
function themeConfig($form) {
    // 基本设置
    $basicFieldset = new Typecho_Widget_Helper_Form_Element_Fieldset('basicFieldset', _t('基本设置'), _t('站点基础配置'), true);
    
    $logoUrl = new Typecho_Widget_Helper_Form_Element_Text('logoUrl', NULL, NULL, _t('站点 LOGO 地址'), _t('在这里填入一个图片 URL 地址, 以在网站标题前加上一个 LOGO'));
    $basicFieldset->addInput($logoUrl);
    ...
    $basicFieldset->addInput($sidebarBlock->multiMode());
    
    $form->addInput($basicFieldset);

    // 动画设置
    $animationFieldset = new Typecho_Widget_Helper_Form_Element_Fieldset('animationFieldset', _t('动画设置'), _t('背景三角形动画配置'), true);
    
    $triangleAnimation = new Typecho_Widget_Helper_Form_Element_Radio('triangleAnimation',
    array('1' => _t('开启'),
    '0' => _t('关闭')),
    '0', _t('三角形动画'), _t('是否开启背景随机三角形动画'));
    $animationFieldset->addInput($triangleAnimation);
    ...
    $codeFieldset->addInput($codeTheme);
    
    $form->addInput($codeFieldset);

    // 底部设置
    $footerFieldset = new Typecho_Widget_Helper_Form_Element_Fieldset('footerFieldset', _t('底部设置'), _t('网站底部相关配置'), true);
    ...
    $footerFieldset->addInput($chinaIcp);
    
    $form->addInput($footerFieldset);

    // 外观设置
    $appearanceFieldset = new Typecho_Widget_Helper_Form_Element_Fieldset('appearanceFieldset', _t('外观设置'), _t('网站外观相关配置'), true);
    ...
    $appearanceFieldset->addInput($backgroundImage);
    ...
    $appearanceFieldset->addInput($darkMode);
    
    $form->addInput($appearanceFieldset);

    // 自定义设置
    $customFieldset = new Typecho_Widget_Helper_Form_Element_Fieldset('customFieldset', _t('自定义设置'), _t('自定义样式和脚本'), true);
    
    $customCss = new Typecho_Widget_Helper_Form_Element_Textarea('customCss', NULL, '', _t('自定义 CSS'), _t('在这里输入自定义 CSS 代码，将应用到整个主题'));
    $customFieldset->addInput($customCss);
    
    $form->addInput($customFieldset);
}
